...
- [ ] Refresh `.github/CONTRIBUTING.md`: it references `.jshintrc`/`.jscsrc`, which do not exist, and
      lists commit types that contradict `.commitlintrc.json`.
- [ ] Refresh the README: the Travis badge and the `require('snyk-policy')` usage predate CircleCI
      and the ESM/CJS dual build.
- [ ] Fix `.github/CODEOWNERS`, which points at `lib/add-exclude.js` while the file is `.ts`.
- [ ] Decide whether `test/` should be linted; ESLint currently runs only over `lib/`.
