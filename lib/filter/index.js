...
function filter(vulns, policy, root) {
  ...
  var vulnIds = vulns.vulnerabilities.map(function (vuln) {
    return vuln.id;
  });

  // check what's be filtered against
  patched.concat(ignored).forEach(function (filtered) {
    var i = vulnIds.indexOf(filtered.id);
    if (i !== -1) {
      vulnIds.splice(i, 1);
    }
  });
